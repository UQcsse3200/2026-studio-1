 ...
 *
 * <p>Each heart represents 10 health points. A player with 100 health
 * therefore has 10 hearts.
 */
public class PlayerStatsDisplay extends UIComponent {
     ...
     *
     * <p>Every 10 health points represents one filled heart.
     * Lost health is represented by empty heart outlines.
     *
    ...
    public void updatePlayerHealthUI(int health) {
        ...
        int visibleHearts =
                (int) Math.ceil((double) health / HEALTH_PER_HEART);

        visibleHearts =
                Math.max(0, Math.min(visibleHearts, MAX_HEARTS));
            ...
            if (i < visibleHearts) {
                ...
                heart.setDrawable(
                        new TextureRegionDrawable(
                                new TextureRegion(currentHeartTexture)
                        )
                );
     ...
     * Custom heart image.
     *
     * <p>Resets the SpriteBatch colour after drawing so that the
     * heart rendering cannot affect other objects in the game.
     */
