...
/**
 * Keyboard navigation for the death screen's two buttons. Mirrors PauseMenuInputComponent -
 * gates on the sibling Display's visibility instead of a separate "paused" flag, since the
 * death screen has no equivalent state component of its own.
 *
 ...
 * already claimed there. Reusing them would fire PauseMenuDisplay's listeners too on every
 * keypress here (e.g. Enter would also trigger PauseMenuActions.resume(), toggling pause
 * state and music even though the pause menu was never opened).
 */
